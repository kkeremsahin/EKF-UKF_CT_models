...
rng(seed);
T = 1; % Sampling Time (s) 
ground_truth_state = [position_gt;velocity_gt];
...
A = kron(eye(2),[1,T;0,1]);
B = kron([T^2/2;T],eye(2));
C = [eye(2),zeros(2)];
Q = sigma_state_filter^2 * eye(2);
...
x = zeros(4,N_samples-1);
x_kp1_hat = [0 ; 0; 27; 0];
x(:,1) = x_kp1_hat;
P_k_hat = diag(100*ones(1,4));
for k = 1:N_samples-1 
    % Measurement update
    S = C*P_k_hat*C'+R;
